...
int i,j;
    ...
    for(i=120,j=1360; i<=580,j>=660; i+=60,j-=60)
    {
        cleardevice();
        setcolor(YELLOW);
        settextstyle(DEFAULT_FONT,HORIZ_DIR,6);
        outtextxy(520,120,"WELCOME");///correction
        setcolor(WHITE);
    ...
    for(i=480; i>=220; i-=20)
    {
        ...
        setcolor(YELLOW);
        settextstyle(DEFAULT_FONT,HORIZ_DIR,6);
        outtextxy(520,120,"WELCOME");
        setcolor(WHITE);
        settextstyle(DEFAULT_FONT,HORIZ_DIR,5);
        outtextxy(630,170,"TO");///correction
        setcolor(YELLOW);
        settextstyle(DEFAULT_FONT,HORIZ_DIR,4);
        outtextxy(480,i,"Hangman Game");
        delay(255);
    }
    ///------portion2----------///
...
a:
    ...
    if(GetAsyncKeyState('S'))
    {
        ...
        for(i=10; i<=100; i+=10)
        {
            ...
            outtextxy(230,i,"Steady");
            delay(120);
        }
        for(i=480; i>=380; i-=20)
        {
            cleardevice();
            outtextxy(100,100,"Ready");
            outtextxy(240,100,"Steady");
